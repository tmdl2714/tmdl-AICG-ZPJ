(()=>{
  const VERSION="featured-ip-focus-platform-aligned-20260616";
  const works=[
  ...
  const injectStyles=()=>{
    ...
    style.textContent=`
      ...
      .featured-ip-focus{position:absolute;left:50%;top:51%;width:min(61vw,930px);aspect-ratio:3/2;opacity:0;pointer-events:none;transform:translate(-50%,-50%) scale(.78);transition:opacity .28s ease,transform .62s cubic-bezier(.18,.78,.18,1);transform-style:preserve-3d}
      ...
      @media(max-width:820px){.featured-ip-overview{top:54%;width:min(90vw,700px);gap:12px}.featured-ip-overview-card{border-radius:8px}.featured-ip-focus{top:52%;width:min(80vw,680px)}.featured-ip-focus::before{width:calc(100% + 22px);height:calc(100% + 14px);border-radius:15px}.featured-ip-arrow{width:36px}.featured-ip-arrow-left{left:max(-46px,-6vw)}.featured-ip-arrow-right{right:max(-46px,-6vw)}}
