(()=>{
  const VERSION="featured-ip-overview-focus-20260616-final";
  const works=[
  ...
  const injectStyles=()=>{
    ...
    style.textContent=`
      ...
      .featured-ip-focus{position:absolute;left:50%;top:52%;width:min(64vw,980px);aspect-ratio:3/2;opacity:0;pointer-events:none;transform:translate(-50%,-50%) scale(.78);transition:opacity .28s ease,transform .62s cubic-bezier(.18,.78,.18,1);transform-style:preserve-3d}
      ...
      .featured-ip-arrow-left{left:max(-70px,-5vw)}
      .featured-ip-arrow-right{right:max(-70px,-5vw)}
      ...
      @media(max-width:820px){.featured-ip-overview{top:54%;width:min(90vw,700px);gap:12px}.featured-ip-overview-card{border-radius:8px}.featured-ip-focus{top:53%;width:min(82vw,720px)}.featured-ip-arrow{width:36px}.featured-ip-arrow-left{left:max(-44px,-6vw)}.featured-ip-arrow-right{right:max(-44px,-6vw)}}
