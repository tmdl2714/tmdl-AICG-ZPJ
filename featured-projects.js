const FEATURED_VERSION = "featured-banner-new-arrival-fit-20260616";
...
function injectFeaturedBannerGalleryStyles() {
  ...
  style.textContent = `
    ...
    .featured-banner-grid{position:absolute;left:50%;top:55%;z-index:3;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));column-gap:clamp(16px,1.55vw,26px);row-gap:clamp(28px,1.9vw,34px);width:min(72vw,1040px);max-height:calc(100vh - 150px);pointer-events:auto;transform:translate(-50%,-50%);perspective:1400px}
    ...
    .featured-banner-card-new-arrival{border:0;background:transparent;box-shadow:none;aspect-ratio:auto;place-items:start center}
    .featured-banner-new-arrival-clip{display:block;width:100%;aspect-ratio:3/2;overflow:hidden;border-radius:9px;clip-path:inset(0 round 9px);-webkit-clip-path:inset(0 round 9px)}
    .featured-banner-card-new-arrival:hover,.featured-banner-card-new-arrival:focus-visible{border-color:transparent;box-shadow:none;filter:brightness(1.03);outline:none}
    ...
    .featured-banner-card-new-arrival img{width:100%;height:auto;min-height:0;object-fit:contain;border-radius:9px;clip-path:inset(0 round 9px);-webkit-clip-path:inset(0 round 9px)}
    ...
    .featured-banner-focus.is-new-arrival{padding:0;border:0;border-radius:9px;background:transparent;box-shadow:none}
    .featured-banner-focus.is-new-arrival img{height:auto;border-radius:9px;clip-path:inset(0 round 9px);-webkit-clip-path:inset(0 round 9px)}
    .featured-banner-focus.is-new-arrival::after{box-shadow:none}
    ...
    @media(max-width:820px){.featured-banner-grid{top:54%;width:min(90vw,700px);column-gap:10px;row-gap:18px}.featured-banner-card{border-radius:7px}.featured-banner-focus{border-radius:8px}}
...
function setupFeaturedBannerGallery(root) {
  ...
  const targetRect = () => {
    ...
    const isNewArrival = activeCard?.classList.contains("featured-banner-card-new-arrival");
    const aspect = isCamera ? 1774 / 887 : isNewArrival ? 3 / 2 : 16 / 9;
    const maxWidthByHeight = Math.max(320, (window.innerHeight - 150) * aspect);
    const width = Math.min(window.innerWidth * .62, isCamera || isNewArrival ? 980 : 1040, maxWidthByHeight);
  ...
  const openCard = (card) => {
    ...
    focus.classList.toggle("is-new-arrival", card.classList.contains("featured-banner-card-new-arrival"));
...
function renderBannerReferencePage() {
  const cards = featuredBannerWorks.map((work,index)=>{
    ...
    const cardClass = index === 0 ? " featured-banner-card-camera" : index === 2 ? " featured-banner-card-new-arrival" : "";
    const content = index === 0 ? `<span class="featured-banner-camera-clip">${image}</span>` : index === 2 ? `<span class="featured-banner-new-arrival-clip">${image}</span>` : image;
    return `<button class="featured-banner-card${cardClass}" type="button" data-banner-work="${index}" aria-label="查看${work.title}">${content}</button>`;
  }).join("");
